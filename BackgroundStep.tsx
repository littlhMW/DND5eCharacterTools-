...
export const TraitSelection: React.FC<TraitSelectionProps> = ({ choice }) => {
  ...
  if (!choice.dynamic && choice.options) {
    if (choice.id.endsWith('-wealth-method')) {
      const cls = classes.find(c => c.id === state.character.classId);
      const classId = cls?.id || 'fighter';
      ...
      const getWealthFormula = (cid: string) => {
        switch (cid) {
          ...
          default: return '5d4 × 10'; // bard, cleric, fighter, paladin, ranger
        }
      };

      const doWealthRoll = (cid: string) => {
        const rollNd4 = (n: number) => {
          let sum = 0;
          for(let i=0; i<n; i++) sum += Math.floor(Math.random() * 4) + 1;
          return sum;
        };
        switch (cid) {
          case 'barbarian': return rollNd4(2) * 10;
          case 'druid': return rollNd4(2) * 10;
          case 'sorcerer': return rollNd4(3) * 10;
          case 'rogue': return rollNd4(4) * 10;
          case 'warlock': return rollNd4(4) * 10;
          case 'wizard': return rollNd4(4) * 10;
          case 'monk': return rollNd4(5);
          default: return rollNd4(5) * 10;
        }
      };

      const [rolledGold, setRolledGold] = useState<number | null>(null);
      const isGoldMode = selectedIds.includes('gold');
      const isStandardMode = selectedIds.includes('standard') || (!selectedIds.includes('gold') && !selectedIds.includes('standard'));

      const handleSetMode = (mode: 'standard' | 'gold') => {
        dispatch({ type: 'SET_TRAIT_SELECTION', payload: { choiceId: choice.id, selectedIds: [mode] } });
      };

      return (
        <div className="mt-4 p-4 bg-white border border-amber-200/50 rounded-md">
          <div className="flex items-center justify-between mb-4 border-b border-stone-100 pb-3">
            <h6 className="font-semibold text-stone-800">{choice.name || '获取装备方式'}</h6>
          ...
          {isStandardMode ? (
            <div className="text-sm font-sans text-stone-600 bg-stone-50 p-4 rounded-xl border border-stone-100">
              <p>你将获得来自<strong>职业</strong>与<strong>背景</strong>提供的默认起始装备。这是最快速的选择。</p>
            </div>
          ) : (
            <div className="text-sm font-sans text-stone-600 bg-amber-50 p-4 rounded-xl border border-amber-100">
              <p className="mb-3">你放弃了职业与背景提供的装备，选择根据公式进行Roll点并得到起始资金（GP），然后自行购买装备。</p>
              <div className="flex items-center gap-3">
                ...
                <button
                  onClick={() => setRolledGold(doWealthRoll(classId))}
                  className="px-4 py-1.5 bg-amber-600 text-white rounded-lg hover:bg-amber-700 transition-colors font-medium shadow-sm flex items-center gap-1"
                >
                  掷骰
                </button>
              </div>
              {rolledGold !== null && (
                <div className="mt-4 p-3 bg-white rounded-lg border border-amber-200 flex items-center justify-between shadow-sm animate-in fade-in slide-in-from-top-2">
                  ...
                  <span className="text-2xl font-bold font-serif text-amber-600">{rolledGold} gp</span>
                </div>
              )}
            </div>
          )}

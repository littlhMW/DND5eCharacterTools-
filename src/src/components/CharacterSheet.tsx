      ...
      <div className="grid grid-cols-1 md:grid-cols-6 gap-6">
        <div className="col-span-1 grid grid-cols-3 md:grid-cols-1 gap-4">
          {(['STR','DEX','CON','INT','WIS','CHA'] as Ability[]).map(ab => {
            ...
            return (
              <div key={ab} className="bg-white border border-stone-200 rounded-lg p-4 text-center shadow-sm flex flex-col relative pb-4">
        ...
        <div className="col-span-1 md:col-span-5 grid grid-cols-1 md:grid-cols-2 gap-8 md:pl-6 pt-4 md:pt-0">
          <div className="space-y-6">
            <div className="grid grid-cols-3 gap-4">
              <div className="bg-white p-4 rounded-lg border border-stone-200 text-center shadow-sm">
          ...
          <div className="space-y-6">
            <div className="bg-white rounded-lg border border-stone-200 p-6 shadow-sm h-full">
...
if (cls.id === 'wizard') {
  ...
  return (
    <div className="min-h-screen bg-[#fffdfc] text-stone-900 font-sans p-4 md:p-8 md:pt-12">
      <div className="max-w-7xl mx-auto bg-[#fffdfc] border border-stone-200 rounded-lg shadow-xl overflow-hidden pb-4 ring-1 ring-stone-900/5">
        {/* Header */}
        <div className="px-8 md:px-12 py-10 flex flex-col md:flex-row justify-between items-end bg-[#fffdfc] relative overflow-hidden">
          ...
          <div>
            ...
            <div className="text-stone-600 flex gap-3 text-sm font-medium flex-wrap">
              <span className="bg-stone-100 px-4 py-1.5 rounded-full whitespace-nowrap border border-stone-200">等级 {c.level}</span>
              <span className="bg-stone-100 px-4 py-1.5 rounded-full whitespace-nowrap border border-stone-200">{subrace?.name || race?.name || '未知种族'}</span>
              <span className="bg-stone-100 px-4 py-1.5 rounded-full whitespace-nowrap border border-stone-200">{subclass?.name || cls?.name || '无职业'}</span>
              <span className="bg-stone-100 px-4 py-1.5 rounded-full whitespace-nowrap border border-stone-200">{bg?.name || '未知背景'}</span>
            </div>
          </div>
          <div className="mt-6 md:mt-0 flex gap-3 flex-wrap">
            <button onClick={() => {
              ...
              alert('角色已保存');
            }} className="px-6 py-2.5 bg-blue-50 text-blue-700 border border-blue-200 rounded-full hover:bg-blue-100 transition font-medium shadow-sm">保存角色</button>
            <button onClick={() => dispatch({ type: 'SET_VIEW', payload: 'landing' })} className="px-6 py-2.5 bg-stone-100 text-stone-700 border border-stone-200 rounded-full hover:bg-stone-200 transition font-medium shadow-sm">主页</button>
                ...
                <ul className="space-y-6">
                  {[
                    ...
                    <li key={`${t.name}-${t.level}-${index}`} className="relative pl-4 before:absolute before:left-0 before:top-2 before:w-1.5 before:h-1.5 before:rounded-full before:bg-stone-300">
                      ...
                      <FormattedDescription text={t.description} className="text-sm font-sans text-stone-600 leading-relaxed block" />
              ...
              <div className="bg-white rounded-lg border border-stone-200 shadow-sm p-8">
                ...
                <ul className="space-y-6">
                  ...
                  {bg && (
                    <li className="relative pl-4 before:absolute before:left-0 before:top-2 before:w-1.5 before:h-1.5 before:rounded-full before:bg-stone-400">
                      ...
                      <FormattedDescription text={bg.feature.description} className="text-sm font-sans text-stone-600 leading-relaxed block" />
                    </li>
                  )}
                </ul>
                ...
                {chosenFeats.length > 0 && (
                  <div className="mt-8 pt-8 border-t border-stone-100">
                    <h3 className="text-2xl font-serif text-amber-600 mb-6 border-b border-stone-100 pb-3">已获得专长</h3>
                    <ul className="space-y-6">
                      {chosenFeats.map((f, i) => (
                        <li key={`feat-${f!.id}-${i}`} className="relative pl-4 before:absolute before:left-0 before:top-2 before:w-1.5 before:h-1.5 before:rounded-full before:bg-amber-400">
                          <strong className="text-stone-900 block mb-1 font-serif text-lg">{f!.name}</strong>
                          <FormattedDescription text={f!.description} className="text-sm font-sans text-stone-600 leading-relaxed block" />
                        </li>
                      ))}
                    </ul>
                  </div>

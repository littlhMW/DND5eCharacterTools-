...
const autoSpells = useMemo(() => {
  ...
  // 子职业提供的自动法术（领域法术等）
  if (subclass?.spells) spells.push(...subclass.spells.filter(s => s.level <= c.level).map(s => ({ id: s.spellId, level: s.level })));
  // 种族和子种族的法术
  const charRace = races.find(r => r.id === c.raceId);
  const charSubrace = charRace?.subraces?.find(sr => sr.id === c.subraceId);
  if (charRace?.spells) spells.push(...charRace.spells.filter(s => (s.level || 0) <= c.level && s.spellId).map(s => ({ id: s.spellId, level: s.level || 0 })));
  if (charSubrace?.spells) spells.push(...charSubrace.spells.filter(s => (s.level || 0) <= c.level && s.spellId).map(s => ({ id: s.spellId, level: s.level || 0 })));
  ...
  const { classChoiceAutoCantripIds, classChoiceAutoLeveledIds } = useMemo(() => {
    const cantrips = new Set<string>();
    const leveled = new Set<string>();
    Object.entries(c.traitSelections).forEach(([key, val]) => {
      if (val && Array.isArray(val)) {
        if (key.match(/-(cantrips)-\d+$/) || key.match(/-(cantrips)$/i)) {
          val.forEach(id => cantrips.add(id));
        } else if (key.match(/-(known|known-spells)-\d+$/) || key.match(/-(known)$/i)) {
          val.forEach(id => leveled.add(id));
        }
      }
    });
    return { classChoiceAutoCantripIds: cantrips, classChoiceAutoLeveledIds: leveled };
  }, [c.traitSelections]);
...
const spellsToDisplay = cls?.id === 'wizard' 
  ...
  if (!spellcasting) {
    return <div className="flex flex-col items-center justify-center p-12 text-stone-500 min-h-[300px]"><h2 className="text-2xl font-serif text-stone-400 mb-2">无施法能力</h2><p className="font-sans text-sm">您选择的职业和等级尚未提供施法能力。</p></div>;
  }

  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      <section>
        <h2 className="text-2xl font-serif text-amber-600 border-b border-stone-200 pb-3 mb-4">法术管理</h2>
        <p className="text-stone-600 font-sans text-sm mb-2">{cls?.name}魔法 · {isPrepared ? '准备施法者' : '已知施法者'}{spellcasting.ability && <> · 施法关键属性：{spellcasting.ability}</>}</p>
      ...
      {availableCantrips.length > 0 && (
        <section>
          <div className="flex justify-between items-end border-b border-stone-200 pb-3 mb-6">
            ...
            {expectedCantrips > 0 ? (
              <span className={`text-sm px-3 py-1 rounded-full font-sans font-medium ${allSelectedCantripIds.filter(id => !autoSpellIds.has(id)).length + classChoiceAutoCantripIds.size >= expectedCantrips ? 'bg-green-100 text-green-700' : 'bg-amber-100 text-amber-700'}`}>
                应选 {expectedCantrips} · 已选 {allSelectedCantripIds.filter(id => !autoSpellIds.has(id)).length + classChoiceAutoCantripIds.size}
              </span>
            ) : (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 md:gap-3">
            {availableCantrips.map(spell => {
              ...
              const isSelected = allSelectedCantripIds.includes(spell.id) || isAuto;
              const currentCount = allSelectedCantripIds.filter(id => !autoSpellIds.has(id)).length + classChoiceAutoCantripIds.size;
              const isDisabled = isAuto || (currentCount >= expectedCantrips && !isSelected);
      ...
      {isKnown && !isMixed && (leveledSpells.length > 0) && (
        <section>
          <div className="flex justify-between items-end border-b border-stone-200 pb-3 mb-6">
            <h3 className="text-xl font-serif text-stone-900">1环及以上法术</h3>
            {expectedKnown > 0 ? (
              <span className={`text-sm px-3 py-1 rounded-full font-sans font-medium ${allSelectedLeveledIds.filter(id => !autoSpellIds.has(id)).length + classChoiceAutoLeveledIds.size >= expectedKnown ? 'bg-green-100 text-green-700' : 'bg-amber-100 text-amber-700'}`}>
                应选 {expectedKnown} · 已选 {allSelectedLeveledIds.filter(id => !autoSpellIds.has(id)).length + classChoiceAutoLeveledIds.size}
              </span>
            ) : (
              <span className="text-sm px-3 py-1 rounded-full font-sans font-medium bg-stone-100 text-stone-700">
                种族/特性赠送
          ...
          {Array.from(new Set(leveledSpells.map(s => s.level))).sort((a: any, b: any) => a - b).map(lvl => (
            <div key={lvl} className="mb-8">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 md:gap-3">
                {leveledSpells.filter(sp => sp.level === lvl).map(spell => {
                  const isAuto = autoSpellIds.has(spell.id);
                  const isSelected = allSelectedLeveledIds.includes(spell.id) || isAuto;
                  const currentCount = allSelectedLeveledIds.filter(id => !autoSpellIds.has(id)).length + classChoiceAutoLeveledIds.size;
                  const isDisabled = isAuto || (expectedKnown > 0 && currentCount >= expectedKnown && !isSelected);
                  return <SpellCard key={spell.id} spell={spell} isSelected={isSelected} isDisabled={isDisabled} isAuto={isAuto}
                    onClick={() => {

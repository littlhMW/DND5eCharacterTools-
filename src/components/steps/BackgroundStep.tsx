...
export function BackgroundStep() {
  ...
  const handleTraitChange = (field: 'personality' | 'specialty' | 'ideals' | 'bonds' | 'flaws', value: string) => {
    dispatch({ type: 'UPDATE_BASIC_INFO', payload: { [field]: value } });
  };

  const rollSpecialty = (options: string[]) => {
    if (options && options.length > 0) {
      const randomIndex = Math.floor(Math.random() * options.length);
      dispatch({ type: 'UPDATE_BASIC_INFO', payload: { specialty: options[randomIndex] } });
    }
  };
      ...
      {selectedBackground && (
        <section className="mt-8 p-6 bg-white rounded-lg border border-stone-200 shadow-sm animate-in fade-in duration-500">
          ...
          {selectedBackground.specialty && (
            <div className="pt-6 border-t border-stone-200 mt-6">
              <h4 className="text-sm font-sans uppercase tracking-[0.15em] text-stone-400 mb-4">{selectedBackground.specialty.name}</h4>
                  ...
                  {selectedBackground.specialty.options.map((opt, index) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <span className={isSelected ? "text-amber-600 font-medium" : ""}>{opt}</span>
                      </li>
                    );
                  })}
                </ul>
            ...
            <div className="pt-6 border-t border-stone-200 mt-6">
              ...
              <div className="space-y-6">
                {[
                  ...
                  <div key={id} className="bg-stone-50 p-5 rounded-md border border-stone-100">
                    <div className="flex items-center justify-between mb-3">
                      <h5 className="font-semibold text-amber-600 flex items-center gap-2">
                        {label}
                        <button

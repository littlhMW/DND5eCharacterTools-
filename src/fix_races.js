...
content = content.replace(
  /name: '高等精灵',[\s\S]*?spells: \[\[\s\S\]*?\]/g,
  match => match.replace("}]", "}], weaponProficiencies: ['长剑', '短剑', '短弓', '长弓']")
);
...
content = content.replace(
  ...
  `{ name: '多才多艺', description: '你获得两个自选技能的熟练项。', \n${halfElfSkillsChoice}\n }`
);
...
const highElfMatch = "spells: [{ level: 0, spellId: '自选法师戏法' }]";
content = content.replace(highElfMatch, "spells: [{ level: 0, spellId: '自选法师戏法' }], weaponProficiencies: ['长剑', '短剑', '短弓', '长弓']");
